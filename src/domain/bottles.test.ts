...
describe('bottles', () => {
  ...
  it('isole les bouteilles par cave', async () => {
    const db = await createTestDb();
    const cave1 = await bootstrapSuperAdmin(db, {
      email: 'a@example.com',
      password: 'x',
      cellarName: 'Cave 1',
    });
    const cave2 = await bootstrapSuperAdmin(db, {
      email: 'b@example.com',
      password: 'x',
      cellarName: 'Cave 2',
    });
...
describe('getBottle / updateBottle / deleteBottle', () => {
  ...
  it('ajoute chaque nouvelle bouteille à la fin de sa clayette (sortOrder croissant)', async () => {
    ...
    const crateId = await createCrate(db, { cellarId, name: 'Clayette 1', capacity: 12 });
    const firstId = await createBottle(db, {
      crateId,
      category: 'wine',
      name: 'Premier',
      quantity: 1,
    ...
    const secondId = await createBottle(db, {
      crateId,
      category: 'wine',
      name: 'Second',
      quantity: 1,
  ...
  it('place une bouteille déplacée vers une autre clayette à la fin de celle-ci', async () => {
    ...
    const movedId = await createBottle(db, {
      crateId: crateAId,
      category: 'wine',
      name: 'À déplacer',
      quantity: 1,
...
describe('reorderBottlesInCrate', () => {
  ...
  it('ignore les bouteilles épuisées (quantité 0) dans la validation de l’ensemble', async () => {
    ...
    const crateId = await createCrate(db, { cellarId, name: 'Clayette 1', capacity: 12 });
    const activeId = await createBottle(db, {
      crateId,
      category: 'wine',
      name: 'Active',
      quantity: 1,
    ...
    const emptyId = await createBottle(db, {
      crateId,
      category: 'wine',
      name: 'Épuisée',
      quantity: 0,
