...
describe('createResetToken / validateResetToken', () => {
  ...
  it('retourne "not_found" pour un token inconnu', async () => {
    const db = await createTestDb();
    expect((await validateResetToken({ db, token: 'inconnu' })).status).toBe('not_found');
  });
  ...
  it('retourne "already_used" pour un token déjà consommé', async () => {
    ...
    const token = await createResetToken({ db, userId });
    await resetPasswordWithToken({ db, token, newPassword: 'nouveau-mot-de-passe' });
...
describe('resetPasswordWithToken', () => {
  it('met à jour le mot de passe et consomme le token', async () => {
    ...
    expect(await verifyPassword({ password: 'nouveau-mot-de-passe', hash: user.passwordHash })).toBe(true);
    expect(await verifyPassword({ password: 'ancien', hash: user.passwordHash })).toBe(false);
  });

  it('rejette un token invalide', async () => {
    const db = await createTestDb();
    await expect(resetPasswordWithToken({ db, token: 'inconnu', newPassword: 'x' })).rejects.toThrow();
  });
});
...
describe('resetPasswordWithToken et les sessions ouvertes', () => {
  it('horodate sessionsValidFrom pour invalider les sessions existantes', async () => {
    ...
    const before = firstRow(await db.select().from(users).where(eq(users.id, userId)));
    ...
    const after = firstRow(await db.select().from(users).where(eq(users.id, userId)));
