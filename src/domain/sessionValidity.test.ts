...
describe('isSessionStillValid', () => {
  it('accepte toute session d’un compte jamais invalidé', () => {
    expect(
      isSessionStillValid({ user: { sessionsValidFrom: null }, issuedAt: '2020-01-01T00:00:00.000Z' }),
    ).toBe(true);
  });
  ...
  it('accepte une session sans date si le compte n’a jamais été invalidé', () => {
    ...
    expect(isSessionStillValid({ user: { sessionsValidFrom: null }, issuedAt: undefined })).toBe(true);
  });

  it('refuse une session émise avant la réinitialisation', () => {
    expect(
      isSessionStillValid({
        user: { sessionsValidFrom: '2026-09-11T12:00:00.000Z' },
        issuedAt: '2026-09-11T11:59:59.000Z',
      }),
    ).toBe(false);
  });
  ...
  it('accepte une session émise après la réinitialisation', () => {
    expect(
      isSessionStillValid({
        user: { sessionsValidFrom: '2026-09-11T12:00:00.000Z' },
        issuedAt: '2026-09-11T12:00:01.000Z',
      }),
    ).toBe(true);
  });
  ...
  it('accepte une session émise exactement à la date d’invalidation', () => {
    ...
    expect(isSessionStillValid({ user: { sessionsValidFrom: at }, issuedAt: at })).toBe(true);
  });

  it('refuse une session sans date sur un compte invalidé', () => {
    expect(
      isSessionStillValid({ user: { sessionsValidFrom: '2026-09-11T12:00:00.000Z' }, issuedAt: undefined }),
    ).toBe(false);
  });
  ...
  it('refuse une date illisible plutôt que de la laisser passer', () => {
    ...
    expect(
      isSessionStillValid({ user: { sessionsValidFrom: '2026-09-11T12:00:00.000Z' }, issuedAt: 'nawak' }),
    ).toBe(false);
    expect(
      isSessionStillValid({ user: { sessionsValidFrom: 'nawak' }, issuedAt: '2026-09-11T12:00:00.000Z' }),
    ).toBe(false);
  });
});
