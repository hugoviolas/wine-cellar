...
describe('isSessionStillValid', () => {
  ...
  it('refuse une session émise avant la réinitialisation', () => {
    expect(
      isSessionStillValid({ sessionsValidFrom: '2026-09-11T12:00:00.000Z' }, '2026-09-11T11:59:59.000Z'),
    ).toBe(false);
  });

  it('accepte une session émise après la réinitialisation', () => {
    expect(
      isSessionStillValid({ sessionsValidFrom: '2026-09-11T12:00:00.000Z' }, '2026-09-11T12:00:01.000Z'),
    ).toBe(true);
  });
  ...
  it('refuse une session sans date sur un compte invalidé', () => {
    expect(isSessionStillValid({ sessionsValidFrom: '2026-09-11T12:00:00.000Z' }, undefined)).toBe(false);
  });
  ...
  it('refuse une date illisible plutôt que de la laisser passer', () => {
    ...
    expect(isSessionStillValid({ sessionsValidFrom: '2026-09-11T12:00:00.000Z' }, 'nawak')).toBe(false);
    expect(isSessionStillValid({ sessionsValidFrom: 'nawak' }, '2026-09-11T12:00:00.000Z')).toBe(false);
  });
});
